...
let processClauses clauses =
    if not quiet.Value then
        printfn "%% process %d clauses." (List.length clauses)

    if print_input.Value then
        List.iter (printfn "  clause %A") clauses

    if not quiet.Value then
        printfn "%% computing fixpoint...."
    ...
    List.iteri
        (fun i clause ->
            if progress.Value then
                printfn "\r%% clause %-5d / %-5d  " i total
    ...
    if print_size.Value then
        printfn "%% size of saturated set: %d." db.Size

    if print_saturated.Value then
        db.Fold (fun () clause -> printfn "  @[<h>%A@]@." clause) ()
    ...
    List.iter
        (fun (vars, lits, neg) ->
            ...
            if not quiet.Value then
                printfn "%% query plan: @[<h>%A@]@." set

            printfn "%% query answer: "

            List.iter
                (fun terms ->
                    Array.iteri (fun i t -> if i > 0 then printfn ", %A" t else printfn "%A" t) terms
                    printfn ";")
                l
...
let addSum symbol =
    ...
    let printer () =
        printfn "%% number of fact with head %s: %d." symbol count.Value
...
let addPattern p =
    ...
    let literal = Parser.parse_literal Lexer.token lexbuf |> DDefault.literalOfAst None
...
let main args =
    ...
    parseFiles () |> List.map DDefault.clauseOfAst |> processClauses
    0
